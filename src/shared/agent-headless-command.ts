...
import { isMuseHeadlessOneShotCommand } from './muse-headless'
...
// the same `--print` one-shot contract, Ante's `--prompt` form, Prime Agent's `--mode`
// forms and Muse's `exec` subcommand need their own matchers.
